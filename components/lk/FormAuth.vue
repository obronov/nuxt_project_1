  ...
  <div class='lk-form'>
    ...
    <form class="lk-form__form" action="/" @submit.prevent="onSubmit($event)">
      <div class="lk-form-group" v-for="(item, index) in formLocal" :key="index">
        <div class="lk-form-group__inner" :class="{isPassword: index == 'password'}">
          <input 
            ...
            :name="index"
            >          
          <button v-if="index == 'password'" class="btnPassword" type="button" @click="showPassword(item)">
    ...
    <button class="lk-btn-text" type="button" @click="$emit('setShowFormRecovery', true)">Забыли пароль?</button>
...
export default {
  ...
  methods:{
    ...
    async onSubmit(event){
      ...
      let response = await fetch(process.env.baseUrl + `user/login`, {
        method: 'POST',
        body: data
      });
